---
name: write-prd
description: Create a product-focused PRD covering personas, user stories, and a detailed end-to-end user flow, then file it as a Linear issue (with "PRD" label) inside a user-specified project. Use when the user wants to write a PRD, draft a product requirements document, or document the product surface of a feature. Supports being run right after drafting a TRD (uses the TRD as primary reference) or after a "grill-me" design session.
---

This skill writes a **product-focused** PRD. It is the user-facing counterpart to a TRD: it describes who the user is, what they want to do, and what they should see and feel at each step. It does **not** describe modules, schemas, APIs, or other implementation details — those belong in a TRD (`write-trd`).

## 1. Identify the trigger path

Ask the user (or infer from context) which path applies:

- **Path A — Post-TRD**: A TRD has just been drafted (in context, or as a Linear issue). Use the TRD as the primary source of truth for problem framing, scope, and feature surface. Your job is to expand the product/user-facing dimensions the TRD doesn't go deep on (personas, end-to-end flow, UX details, success metrics).
- **Path B — Post-grill-me**: A deep design-thinking session just happened. Use that conversation as the primary source. Skim it for user pain, decisions, and feature scope, then fill the gaps via interview.
- **Path C — From scratch**: No prior context. Run a product-focused interview yourself (problem, users, goals, flows) before drafting.

If a TRD exists, fetch it (Linear MCP `get issue by ID`) only if it's not already in context. Capture its Linear issue identifier — you'll cross-reference it in the PRD.
...
For each question, recommend an answer based on what's already known. If a question is answerable from the TRD, the grill-me transcript, or the codebase, answer it yourself instead of asking.
...
## 4. File the PRD in Linear

Ask the user which Linear **project** to file the PRD under. Then create a Linear issue using the Linear MCP:

- **Title**: short, product-flavored title for the feature (not a technical name)
- **Description**: the full PRD content using the template below
- **Project**: the project the user provided
- **Label**: "PRD" (create the label if it does not already exist in the workspace)

**Cross-reference handling (if a related TRD exists):**

- Include the TRD's Linear identifier in the PRD's `## Related TRD` section.
- After the PRD is created, update the TRD issue's description to add a `## Related PRD` line at the top pointing back to the new PRD identifier. The two issues are **siblings** — do not set either as the parent of the other.

After creating the issue, share the Linear issue URL with the user.

<prd-template>

## Related TRD

The Linear identifier of the related TRD, if any. Otherwise: "None."
