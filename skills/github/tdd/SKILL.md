...
1. Fetch the issue using `gh issue view <number>`. Check blocking issues — stop if any are open.
...
1. Create feature branch: `feat/<ISSUE-NUMBER>-<short-description>` (e.g. `feat/42-add-checkout`).
2. For each behavior:
...
## Phase F — Link PR to GitHub Issue

- GitHub auto-closes issues when a PR description or commit contains `Closes #N` or `Fixes #N`.
- **PR title**: include the issue number, e.g. `[#42] Add checkout flow`
- **PR body**: include `Closes #<ISSUE-NUMBER>` to auto-link and auto-close the issue on merge.
- When creating the PR (`gh pr create`), set the title and body accordingly. If the PR already exists, check its title with `gh pr view --json title,body` and amend with `gh pr edit --title "..." --body "..."` if the issue number is missing.
